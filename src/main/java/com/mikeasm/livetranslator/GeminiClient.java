...
public final class GeminiClient {
    ...
    private static final String RULES = """
            Правила:
            1. Передавай то, что действительно прозвучало, не пересказывая и
               ничего не добавляя.
            ...
            3. Если фрагмент не разобрать, поставь [неразборчиво]. Выдуманная
               фраза хуже пропуска.
            4. Названия и термины пиши общепринятым написанием.
            5. Не отвечай на содержание сказанного и не добавляй ничего от себя:
               ни вводных слов, ни пояснений.
            6. Одна строка — одна законченная реплика. Не дроби слова между
               строками и не обрывай строку посреди слова.
            7. Если разборчивой речи в записи нет, ответь одним словом: ТИШИНА.
               Не придумывай реплики и не обращайся ко мне с просьбами.
            """;
    ...
    private static final String TRANSLATE = """
            Ты синхронный переводчик. Прослушай запись, распознай речь и выведи
            её точный, естественный перевод на %s. Говорящих может быть
            несколько, и язык может меняться посреди разговора.
    ...
    static List<Line> parse(String answer) {
        ...
        if (open) found.add(line(startMs, text, original));
        found.removeIf(line -> isChatter(line.text()));
        return join(found);
    }
    ...
    /**
     * Обращения модели к нам самим.
     * <p>
     * На записи без речи модель отвечает не молчанием, а просьбой прислать
     * запись — и эта просьба уезжает в окно и в расшифровку как перевод.
     * Просить модель отвечать пустотой оказалось хуже: она начинала выдумывать
     * правдоподобную встречу, используя термины из нашего же словаря, и отличить
     * такое от настоящего перевода невозможно. Поэтому на тишину у неё есть
     * условленное слово, которое мы и выбрасываем: заметная ошибка лучше
     * незаметной.
     */
    /**
     * Условленный ответ на запись без речи — и рассуждения вокруг него.
     * <p>
     * Модель не всегда отвечает одним словом: бывает «в соответствии с правилом
     * №7: ТИШИНА». Слово заглавными в живой речи не встречается, поэтому ловим
     * его где угодно в строке.
     */
    private static final Pattern SILENCE = Pattern.compile("(?U)\\bТИШИНА\\b");

    private static final Pattern CHATTER = Pattern.compile(
            "(?iuU)(предоставьте|пришлите|прикрепите|отправьте).{0,40}"
                    + "(аудио|запись|текст|транскрипт|файл|материал)"
                    + "|не\\s+(могу|смогу)\\s+(выполнить|сделать)\\s+перевод"
                    + "|нет\\s+(аудио|записи|исходных)"
                    + "|отсутству\\w+\\s+(как\\w+-либо\\s+)?(речь|звук|аудио)"
                    + "|в\\s+соответствии\\s+с\\s+правил"
                    + "|(фрагмент|запись)\\w*\\s+(пуст|без\\s+речи)");

    static boolean isChatter(String text) {
        return SILENCE.matcher(text).find() || CHATTER.matcher(text).find();
    }
