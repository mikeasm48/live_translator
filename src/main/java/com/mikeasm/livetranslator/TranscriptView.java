...
public interface TranscriptView {
    ...
    /**
     * Чем приложение занято прямо сейчас: слушает речь, ждёт ответа модели,
     * стоит в тишине.
     * <p>
     * Отличается от {@link #status(String)} тем, что не гаснет: это не новость,
     * а положение дел. Приёмник, которому это безразлично, переопределения
     * может не замечать.
     */
    default void state(String state) {}
}
